...
app.get('/config', (req, res) => {
  res.json({
    ...
    certPath: config.certPath ? 'configurado' : '',
    certConfigurado: Boolean(config.certPath),
    cscConfigurado: Boolean(config.csc),
  });
});

// Escribe datos del cliente (certificado/CSC/ambiente) en el .env propio y lo
// recarga en memoria. Lo invoca Django desde la pantalla de configuración SIFEN.
app.post('/config', (req, res) => {
  const fs = require('fs');
  const { ambiente, certPath, certPassword, csc, cscId } = req.body || {};
  try {
    let contenido = fs.readFileSync(__dirname + '/.env', 'utf8');
    const cambios = {};
    if (ambiente !== undefined) cambios.SIFEN_AMBIENTE = String(ambiente);
    if (certPath !== undefined) cambios.CERT_PATH = certPath ? String(certPath) : '';
    if (certPassword !== undefined) cambios.CERT_PASSWORD = certPassword ? String(certPassword) : '';
    if (csc !== undefined) cambios.CSC = csc ? String(csc) : '';
    if (cscId !== undefined) cambios.CSC_ID = String(cscId);

    for (const [key, value] of Object.entries(cambios)) {
      const re = new RegExp(`^${key}=.*$`, 'm');
      if (re.test(contenido)) {
        contenido = contenido.replace(re, `${key}=${value}`);
      } else {
        contenido += `\n${key}=${value}`;
      }
    }
    fs.writeFileSync(__dirname + '/.env', contenido, 'utf8');
    config.reload();
    res.json({ ok: true, ambiente: config.ambiente, mock: config.mock });
  } catch (err) {
    handleError(res, err);
  }
});
